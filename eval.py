...
if __name__ == "__main__":
    ...
    if instances_path.is_file() and solutions_path.is_file():
        ...
        instances = {
            f.stem: CVRPInstance.from_file(f)
            for f in instances_path.iterdir()
            if f.stem in solutions
        }
    else:
    ...
    results = [
        {stem: evaluate_solution(instances[stem], solutions[stem])} for stem in stems
    ]
